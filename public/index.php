...
function checkSystemRequirements()
{
    ...
    if (version_compare($phpVersion, '8.2.0', '>=')) {
        ...
        $requirements['php_version']['message'] = "PHP {$phpVersion} (Required: >= 8.2.0)";
        $requirements['php_version']['fix'] = "Ubuntu: sudo apt-get install php8.2 php8.2-cli";
    }
    ...
    if (empty($missingExtensions)) {
        ...
        $requirements['php_extensions']['message'] = "All required PHP extensions installed ✓";
    } else {
        $requirements['php_extensions']['message'] = "Missing extensions: " . implode(', ', $missingExtensions);
        $requirements['php_extensions']['fix'] = "Ubuntu: sudo apt-get install " . implode(' ', array_map(fn($e) => "php8.2-{$e}", $missingExtensions));
    }
    ...
    if ($composerExecutable) {
        ...
        if (preg_match('/Composer version (\d+\.\d+\.\d+)/i', $composerOutput, $matches)) {
            ...
            if (version_compare($composerVersion, '2.0.0', '>=')) {
                ...
                $requirements['composer_version']['fix'] = "composer self-update";
            }
        }
    } else {
        $requirements['composer_version']['message'] = "Composer not found";
        $requirements['composer_version']['fix'] = "curl -sS https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer";
    }
