...
export const QuickSEO = {
    ...
    /**
     * Product page SEO with product-specific keywords
     */
    product: (productName: string, description: string, canonical: string, price?: string) => {
        const productTitle = price ? `${productName} - Starting from ${price}` : productName;

        return createPageSEO({
            title: productTitle,
            description,
            canonical,
            keywords: [productName, 'eSIM plan', 'data package', 'travel connectivity']
        });
    },

    /**
     * Country/Destination page SEO
     */
