...
const Navbar = () => {
    ...
    const navData = {
        secondNavbar: {
            ...
            navlinks: [
                {
                    label: 'home.nav.whatIsEsim', // t('nav.whatIsEsim')
                    href: '/what-is-esim'
                },
                {
    ...
    const getNavbarClasses = () => {
        ...
        if (isMobileDevice()) {
            return `${baseClasses} bg-[#022ffd] shadow-md backdrop-blur-sm`;
        }

        // Desktop behavior
        return `${baseClasses} ${isScrolled ? 'bg-[#022ffd] shadow-md backdrop-blur-sm' : 'bg-white'}`;
    };
                ...
                <div className='flex items-center justify-between'>
                            ...
                            {navData?.secondNavbar.navlinks.map((item, i) => (
                                <div key={i} className='group relative'>
                                    {item.href ? (
                                        <Link
                                            href={item.href}
                                            className={`py-2 text-sm font-medium transition-colors duration-300 hover:text-white md:text-xs lg:text-sm ${getTextColorClass()}`}>
                                            {t(`${item.label}`)}
                                        </Link>
                                    ) : (
                                        <div>
                                            <button
                                                ...
                                                className={`flex items-center py-2 text-sm font-medium transition-colors duration-300 hover:text-white ${getTextColorClass()}`}
                    ...
                    <div className='hidden flex-shrink-0 items-center gap-3 lg:flex xl:gap-4'>
                        ...
                        <button
                            ...
                            className={`hidden transform items-center gap-3 rounded-full px-6 py-2 font-semibold shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-xl focus:ring-4 focus:outline-none xl:inline-flex ${
                                isScrolled
                                    ? 'border border-gray-200 bg-white text-gray-900 hover:border-gray-300 hover:bg-gray-50 focus:ring-gray-500/30'
                                    : 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white hover:from-purple-700 hover:to-indigo-700 focus:ring-purple-500/30'
                            }`}>
                            <span className='text-sm'>See packs</span>
                            <FaPlaneDeparture className={`text-lg ${isScrolled ? 'text-gray-700' : 'text-white'}`} />
                        </button>
