...
const AllDestinations = () => {
    ...
    const unifiedDestinations = useMemo((): UnifiedDestination[] => {
        const countryDestinations: UnifiedDestination[] = countries.map((country) => ({
            id: `country-${country.id}`,
            name: country.name,
            price: `From ${userRedux?.currency?.symbol || '$'}${country.start_price.toFixed(2)}`,
            image: country.image,
        ...
        const regionDestinations: UnifiedDestination[] = regions.map((region) => ({
            id: `region-${region.id}`,
            name: region.name,
            price: `From ${userRedux?.currency?.symbol || '$'}${region.start_price.toFixed(2)}`,
            image: region.image,
