                        ...
                        <div className='space-y-2 text-sm text-gray-700'>
                            ...
                            <p>
                                ...
                                {t('profile.pages.esimDetails.currency.format', {
                                    price: esim.order.activation_details.price,
...
const MyEsimDetails = () => {
    ...
    function ExpiryInfo({ esim }: ExpiryInfoProps) {
        ...
        if (!isExpired) {
            ...
            const parts = [];
            if (diffDays > 0) {
                parts.push(t('profile.pages.esimDetails.timeLeft.days', { count: diffDays }));
            }
            if (diffHours > 0) {
                parts.push(t('profile.pages.esimDetails.timeLeft.hours', { count: diffHours }));
            }
            if (diffMinutes > 0) {
                parts.push(t('profile.pages.esimDetails.timeLeft.minutes', { count: diffMinutes }));
            }
            ...
            <div className='flex items-center gap-2 text-gray-700'>
                ...
                <span className={`text-sm ${isRed ? 'font-semibold text-red-500' : ''}`}>
                    {isExpired
                        ...
                        : t('profile.pages.esimDetails.timeLeft.left', { time: timeLeftString })}
                </span>
            </div>
        );
    }
                            ...
                            <div className='flex items-center gap-2'>
                                ...
                                <span
                                    className={`text-sm font-medium ${
                                        esim.status === 'EXPIRED' ? 'text-red-600' : 'text-green-600'
                                    }`}>
                                    {esim.status}
                                </span>
                            </div>
